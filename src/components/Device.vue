...
<div class="Device">
	...
	<div class="search">
		<select>
			<option value="类型设备">类型设备</option>
			<option value="类型设备2">类型设备2</option>
		</select>
		...
		<!-- 1.搜索功能双向绑定获取数据 -->
		<input type="text" name="text" placeholder="搜索设备" v-model="search">
	</div>
	...
	<span v-if="activeName!=null">
		<div v-for="(item, index) in list" :key="index">
			<div class="working" v-if="activeName==item.status">	
				<ul>
					<li class="strong">XXX类型设备</li>

					<!-- 设备详情跳转路由 -->
					<router-link to="/Device_details">
						<li class="work" :class="{work:item.status == '工作中',standby:item.status == '待机中',deployed: item.status == '部署中'}">{{item.status}}</li>
					</router-link>
				</ul>
	...
	<div class="menu" id="abc">
		...
		<router-link to='/Device'>
			<img src="../assets/image/device_2.png">
		</router-link>
